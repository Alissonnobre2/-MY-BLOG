...
section[data-testid="stSidebar"] input, section[data-testid="stSidebar"] textarea, section[data-testid="stSidebar"] select{ color: var(--navy) !important; }
...
section[data-testid="stSidebar"] button{ background:var(--gold) !important; color:var(--navy) !important; border:none !important; font-weight:700 !important; border-radius:30px !important; }
...
.card{ background:white; border:1px solid #ECEEF2; border-radius:16px; padding:28px; box-shadow:0 6px 18px rgba(27,42,74,.05); margin-bottom:22px; }
.card h2, .card h3{ font-family:'Playfair Display', serif; color:var(--navy); }
...
.pill{ display:inline-block; background:var(--gold-light); color:#8A6A1E; border:1px solid #F0DBA0; padding:4px 14px; border-radius:30px; font-size:12px; font-weight:700; margin-right:6px; }
.pill.status-progress{ background:#FDEFE0; color:#B5631C; border-color:#F3C895; }
.pill.status-done{ background:#E7F4EA; color:#2F7A43; border-color:#BFE3C8; }
...
.caption-muted{ text-align:center; color:var(--muted); font-size:13px; }
...
with st.sidebar:
    ...
    if st.session_state.edit_mode:
        ...
        with st.expander("🎓 Manage Academic Work"):
            ...
            work["status"] = st.selectbox("Status", ["Completed", "In Progress"], index=0 if work["status"] == "Completed" else 1, key=f"{work_key}_status")
...
if st.session_state.page == "HOME":
    ...
    # BANNER SUPERIOR ATUALIZADO COM AS SUAS FOTOS REAIS
    p1, p2, p3 = st.columns(3)
    with p1:
        st.markdown('<div class="home-photo">', unsafe_allow_html=True)
        st.image("WhatsApp Image 2026-06-30 at 20.02.41 (2).jpeg", use_container_width=True, caption="Touchstone 3 Textbook")
        st.markdown("</div>", unsafe_allow_html=True)
    with p2:
        st.markdown('<div class="home-photo">', unsafe_allow_html=True)
        st.image("WhatsApp Image 2026-06-30 at 20.02.41.jpeg", use_container_width=True, caption="Classroom Observation")
        st.markdown("</div>", unsafe_allow_html=True)
    with p3:
        st.markdown('<div class="home-photo">', unsafe_allow_html=True)
        st.image("WhatsApp Image 2026-06-30 at 20.02.41 (1).jpeg", use_container_width=True, caption="Digital Audio Activities")
        st.markdown("</div>", unsafe_allow_html=True)

    st.write("")
    
    # OS 3 QUADRADOS DE STATUS FORAM REMOVIDOS DAQUI DIRETAMENTE
        ...
        for key, work in st.session_state.ACADEMIC_WORK.items():
            with st.container(border=True):
                ...
                if st.button("Read more →", key=f"home_read_{key}"):
                    if key == "creed": go_to("PEDAGOGICAL CREED")
                    elif key == "pip": go_to("PEDAGOGICAL INTERVENTION PLAN")
                    elif key == "observation": go_to("OBSERVATION REPORT")
                    st.rerun()
    ...
    with right:
        st.image("https://picsum.photos/seed/onlyenglishtime-hero/700/900", use_container_width=True)
...
# =========================================================
    ...
    st.write("Section content for profile.")
